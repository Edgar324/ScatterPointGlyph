...
VarSelectionWidget::VarSelectionWidget() {
...
void VarSelectionWidget::SetData(vector<QString>& names, vector<QColor>& colors) {
    var_names_ = names;
    var_colors_ = colors;
...
void VarSelectionWidget::GetSelection(vector<bool>& is_selected) {
    is_selected.clear();
...
void VarSelectionWidget::UpdateWidget() {
    item_model_->clear();
...
void VarSelectionWidget::OnItemClicked(QModelIndex index) {
    if (index.column() == 0) {
