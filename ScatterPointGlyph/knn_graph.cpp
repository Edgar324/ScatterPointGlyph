...
bool cmp_knn_t(const KnnNode& lhs, const KnnNode& rhs) {
...
void KnnGraph::BuildGraph(int nearest_neighbors, const vector<vector<double>>& values, const vector<double>& weights, vector<vector<bool>>& connecting) {
    ...
    for (int i = 0; i + 1 < node_num; i++)
        for (int j = i + 1; j < node_num; j++) {
            ...
            if (temp_dis > max_distance_) max_distance_ = temp_dis;
        }
    ...
    for (int i = 0; i < node_num; i++) {
        ...
        make_heap(knn_vec.begin(), knn_vec.end(), cmp_knn_t);

        for (int j = nearest_neighbors + 1; j < node_num; j++) {
            knn_vec.push_back(KnnNode{ dis_mat[i][j], j });
            push_heap(knn_vec.begin(), knn_vec.end(), cmp_knn_t);
            pop_heap(knn_vec.begin(), knn_vec.end(), cmp_knn_t);
            knn_vec.pop_back();
        }
